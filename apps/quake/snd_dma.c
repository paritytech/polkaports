...
void S_UpdateAmbientSounds (void)
{
	...
	if (!snd_ambient || !ambient_level.value || !cl.worldmodel)
		return;

// calc ambient sound levels
	if (!CM_TouchedLeafs (listener_origin, vec3_origin, vec3_origin, &l, 1, NULL))
		return;
	...
	for (ambient_channel = 0 ; ambient_channel< NUM_AMBIENTS ; ambient_channel++)
	{
		...
		vol = ambient_level.value * CM_LeafAmbientLevel (l, ambient_channel);
		if (vol < 8)
			vol = 0;
