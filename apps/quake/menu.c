...
void M_DrawTransPic (int x, int y, struct mpic_s *pic)
{
...
byte translationTable[VID_GRADES*256];
struct model_s *player_mdl;
...
void M_BuildTranslationTable(int top, int bottom)
{
	...
	player_mdl = Mod_ForName ("progs/player.mdl", false);

	// slot -1 is reserved for faked player model
	R_TranslatePlayerSkin (-1, player_mdl, 0, top, bottom);
...
void M_Main_Key (int key)
{
	...
	case K_ESCAPE:
		...
		if (cls.demonum != -1 && !cls.demoplayback && Com_ClientState() != ca_connected)
			CL_NextDemo ();
		break;
...
void M_Setup_Draw (void)
{
	...
	refdef.x = ((vid.width - 320)>>1) + 169;
	refdef.y = 64;
	refdef.width = 55;
	refdef.height = 75;
	refdef.fov_x = 41;
	refdef.fov_y = CalcFov( refdef.fov_x, refdef.width, refdef.height );
	refdef.time = realtime;
	refdef.oldtime = realtime;

	memset( &entity, 0, sizeof( entity ) );

	entity.model = player_mdl;
	...
	entity.frame = 0;
	entity.oldframe = 0;
	entity.framelerp = 0.0;
	entity.colormap = translationTable;
	entity.angles[1] = yaw++;
	if ( ++yaw > 360 )
		yaw -= 360;
...
void M_AdjustSliders (int dir)
{
	...
	case 3:	// screen size
		...
		v = bound (30, v, 120);
		Cvar_SetValue (&scr_viewsize, v);
		break;
	case 4:	// gamma
		v = v_gamma.value - dir * 0.05;
		v = bound (0.5, v, 1);
		Cvar_SetValue (&v_gamma, v);
		break;
	case 5:	// mouse speed
		v = sensitivity.value + dir * 0.5;
		v = bound (1, v, 11);
		Cvar_SetValue (&sensitivity, v);
		break;
	case 6:	// music volume
#ifdef _WIN32
		v = bgmvolume.value + dir * 1.0;
#else
		v = bgmvolume.value + dir * 0.1;
#endif
		v = bound (0, v, 1);
		Cvar_SetValue (&bgmvolume, v);
		break;
	case 7:	// sfx volume
		v = volume.value + dir * 0.1;
		v = bound (0, v, 1);
...
void M_Options_Draw (void)
{
	...
	M_Print (16, 128, "              Freelook");
	M_DrawCheckbox (220, 128, cl_freelook.value);

	M_Print (16, 136, "      Graphics Options");
...
/*
	...
	Bobbing items			on/off
	Show framerate			on/off
	Fast Sky				on/off
	.lit Files				on/off
*/
...
void M_Gfx_Draw (void)
{
	int				y;
	static struct mpic_s	*p = NULL;
	extern cvar_t r_lerpmodels, r_loadlits;
	...
	y = 32;
	M_Print (16, y, "   Frame interpolation"); M_DrawCheckbox (220, y, r_lerpmodels.value); y += 8;
	M_Print (16, y, "         Bobbing items"); M_DrawCheckbox (220, y, cl_bobbingitems.value); y += 8;
	M_Print (16, y, "        Show Framerate"); M_DrawCheckbox (220, y, show_fps.value); y += 8;
	M_Print (16, y, "              Fast Sky"); M_DrawCheckbox (220, y, r_fastsky.value); y += 8;
	M_Print (16, y, "            .lit Files"); M_DrawCheckbox (220, y, r_loadlits.value); y += 8;
...
void M_Gfx_Set (void)
{
	...
	{
		case 0:
			v = !(int)r_lerpmodels.value;
			Cvar_SetValue (&r_lerpmodels, v);
			break;

		case 1:
			v = !(int)cl_bobbingitems.value;
			Cvar_SetValue (&cl_bobbingitems, v);
			break;

		case 2:
			v = !(int)show_fps.value;
			Cvar_SetValue (&show_fps, v);
			break;

		case 3:
			v = !(int)r_fastsky.value;
			Cvar_SetValue (&r_fastsky, v);
			break;

		case 4:
			v = !(int)r_loadlits.value;
			Cvar_SetValue (&r_loadlits, v);
			break;
...
#define	NUMCOMMANDS	(sizeof(bindnames)/sizeof(bindnames[0]) - 1)
...
void M_Menu_GameOptions_f (void)
{
	...
	m_entersound = true;
	if (maxplayers == 0)
		maxplayers = Com_ServerMaxclients ();
	if (maxplayers < 2)
		maxplayers = 4;
}
