...
void Con_StartDebugLog (char *file);
...
#define MAXGAMEDIRLEN	1000
	...
	if (con_debuglog)
	{
		...
		Con_StartDebugLog (va("%s/qconsole.log", com_gamedir));
	}
...
#ifndef _WIN32
static int condebug_fd;
#else
static FILE *condebug_fd;
#endif
...
{
#ifndef _WIN32
    condebug_fd = open(file, O_WRONLY | O_CREAT | O_APPEND, 0666);
#else
	condebug_fd = fopen (file, "at");
#endif
}
...
#ifndef _WIN32
    write(condebug_fd, data, strlen(data));
#else
	fwrite (data, strlen(data), 1, condebug_fd);
	fflush (condebug_fd);
#endif
}
...
void Con_DrawNotify (void)
{
	...
	if (key_dest == key_message)
	{
		...
		while(chat_buffer[t])
		{
			Draw_Character ( x<<3, v, chat_buffer[t]);
			x++;
			t++;
		}
...
// draw the text
	...
	y = lines - 24;
	row = con_current - con_backscroll;
	...
	for (i= 0 ; i<rows ; i++, y-=8, row-- )
	{
		...
		text = con_text + ((unsigned)row % con_totallines)*con_linewidth;
